...
const firstName = computed(() => session.currentUser?.name?.split(' ')[0] || 'U¿ytkowniku')
...
const handleAddClientClick = (e?: Event) => {
  ...
  if (canAddClient.value) {
    ...
    alert(`Brak uprawnieñ do "Strefa Klienta". Twoja rola: ${session.currentUser?.role || 'Nieznana'}`)
  }
}
...
const newsItems = computed(() => {
  if (Array.isArray(newsStore.feed) && newsStore.feed.length > 0) {
    return newsStore.feed.map((item) => {
      ...
      else if (item.category === 'SALES') { tag = 'SPRZEDA¯'; color = 'bg-green-500' }
...
const goalLabel = computed(() => 'Osi¹gniêty cel miesiêczny')
  ...
  <div class="view-transition pb-4 space-y-3">
    <div v-if="viewMode === 'hub'" class="w-full pt-2">
      <div class="rounded-card shadow-card-hover border p-6 mb-4 flex flex-col md:flex-row justify-between items-center gap-4" style="background: linear-gradient(135deg, #001f3d 0%, #002a52 50%, #003366 100%); border-color: #003366;">
        <div class="flex-1">
          <h1 class="font-serif font-bold text-3xl text-white mb-2 tracking-tight">Dzieñ dobry, {{ firstName }}</h1>
        ...
        <div v-else @click="handleAddClientClick" class="crm-tile h-36 opacity-60 grayscale cursor-not-allowed relative overflow-hidden bg-slate-100 border border-slate-200">
           ...
           <div class="relative z-10 w-full px-4 pt-6 pb-4 h-full flex flex-col justify-between">
            ...
            <div>
              ...
              <p class="crm-tile-desc text-xs text-slate-400 font-medium">Brak uprawnieñ</p>
            </div>
          </div>
        </div>
        ...
        <RouterLink to="/app/clients" class="crm-tile h-36 group relative overflow-hidden bg-slate-100 border border-slate-200">
          ...
          <div class="relative z-10 w-full px-4 pt-6 pb-4 h-full flex flex-col justify-between">
            ...
            <div>
              ...
              <p class="crm-tile-desc text-xs text-slate-500 font-medium">Zarz¹dzaj umowami</p>
            </div>
          </div>
        </RouterLink>
        ...
        <RouterLink v-if="userRole !== 'SALES'" to="/app/recruitment" class="crm-tile h-36 group relative overflow-hidden bg-slate-100 border border-slate-200">
          ...
          <div class="relative z-10 w-full px-4 pt-6 pb-4 h-full flex flex-col justify-between">
            ...
            <div>
              ...
              <p class="crm-tile-desc text-xs text-slate-500 font-medium">Zarz¹dzaj kandydatami</p>
            </div>
          </div>
        </RouterLink>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 items-stretch">
        <div class="relative flex flex-col bg-white rounded-card shadow-card border border-slate-100 p-6 group transition-all duration-300 overflow-hidden hover:shadow-card-hover hover:-translate-y-0.5">
          ...
          <div class="relative z-10 flex flex-col flex-1">
            <div class="flex justify-between items-center mb-4">
              <h3 class="font-bold text-slate-700 uppercase text-[10px] tracking-wider border-l-4 border-primary pl-3 transition-colors duration-300 group-hover:text-primary">Najbli¿sze Wydarzenia</h3>
            </div>
            
            <div class="space-y-3">
              <div v-if="upcomingEvents.length === 0" class="text-center py-6 text-slate-400 italic text-sm">Brak nadchodz¹cych wydarzeñ</div>
            ...
            <div class="mt-4 text-center">
              <RouterLink to="/app/calendar" class="text-[10px] font-bold text-primary hover:text-slate-800 uppercase tracking-widest transition group/link inline-flex items-center relative z-10">
                Pe³ny Kalendarz <span class="ml-1 group-hover/link:translate-x-1 transition-transform">›</span>
              </RouterLink>
            </div>
          </div>
        </div>
        ...
        <div class="relative flex flex-col bg-white rounded-card shadow-card border border-slate-100 p-6 group transition-all duration-300 overflow-hidden hover:shadow-card-hover hover:-translate-y-0.5">
          ...
          <div class="relative z-10 flex flex-col flex-1">
            <div class="flex justify-between items-center mb-4">
              <h3 class="font-bold text-slate-700 uppercase text-[10px] tracking-wider border-l-4 border-primary pl-3 transition-colors duration-300 group-hover:text-primary">Aktualnoœci</h3>
            </div>
              ...
              <div class="relative overflow-hidden flex-1" style="min-height:60px" @click="newsItems.length > 0 && openNewsModal(newsItems[currentNewsIndex])">
                <TransitionGroup name="news-slide" tag="div" class="h-full w-full relative">
                  ...
                  >
                    ...
                    <div class="news-content-area text-slate-700 leading-relaxed text-sm line-clamp-3" :class="{'text-white': newsItems[currentNewsIndex]?.title?.toUpperCase().includes('KONKURS')}" v-html="newsItems[currentNewsIndex]?.content || '<i>Brak dodatkowej treœci</i>'"></div>
                    </div>
                  </div>
                  <div v-else key="empty" class="flex items-center justify-center h-full text-slate-400 italic">Brak aktualnoœci</div>
                </TransitionGroup>
              </div>
              
              <div v-if="newsItems.length > 0" class="mt-3 text-center pt-2 flex flex-col items-center gap-1.5">
                <span class="text-[10px] text-slate-400 font-bold uppercase tracking-widest group-hover:text-primary transition-colors cursor-pointer" @click="newsItems.length > 0 && openNewsModal(newsItems[currentNewsIndex])">Kliknij aby czytaæ wiêcej</span>
    ...
    <div v-else class="space-y-8 max-w-7xl mx-auto pt-6">
      <div class="rounded-card shadow-card-hover border p-8" style="background: linear-gradient(135deg, #001f3d 0%, #002a52 50%, #003366 100%); border-color: #003366;">
            ...
            <div class="relative w-full md:w-56">
              ...
              <div class="relative">
                <select v-model="viewScope" class="w-full bg-slate-800 border border-slate-700 text-white py-2 pl-4 pr-10 rounded-lg text-sm font-bold focus:outline-none focus:border-primary cursor-pointer appearance-none transition hover:bg-slate-750">
                  ...
                  <option v-if="userRole === 'ADMIN'" value="role:MANAGER">Widok: Menad¿erowie</option>
                  ...
                  <option v-if="userRole === 'DIRECTOR'" value="team">Widok: Zespó³</option>
                </select>
        ...
        >
          ...
          <span class="leading-tight text-center">Mój Zespó³</span>
        </button>
        <button 
        ...
        >
          ...
          <span class="leading-tight text-center">Zaleg³oœci P³atnicze</span>
        </button>
        <button 
      ...
      <div class="bg-surface rounded-card shadow-card border border-slate-100 overflow-hidden mb-8" v-if="showCalculationsTab">
        <div class="bg-slate-50 border-b border-slate-100 p-2 flex items-center shadow-sm shrink-0">
          ...
          <span class="text-[10px] text-slate-500 font-bold bg-white border border-slate-200 px-4 py-2 rounded-full uppercase tracking-wide shadow-sm mr-4 shrink-0">Sortowanie: Czas do wygaœniêcia</span>
        </div>
          ...
          <table class="w-full text-sm text-left">
            <thead class="text-xs text-slate-400 uppercase bg-slate-50 border-b border-slate-200 font-bold tracking-wider">
              <tr>
                ...
                <th class="px-4 py-4">Wa¿noœæ PLUS</th>
      ...
      <div class="bg-surface rounded-card shadow-card border border-slate-100 overflow-hidden mb-12" v-if="showArrearsTab">
        <div class="bg-slate-50 border-b border-slate-100 p-2 flex items-center shadow-sm shrink-0">
          <div class="flex items-center gap-3 ml-4">
            ...
            <div class="flex flex-col sm:flex-row sm:items-baseline sm:gap-4">
              <h3 class="font-bold text-slate-800 text-lg tracking-tight">Zaleg³oœci P³atnicze</h3>
          ...
          <table class="w-full text-sm text-left">
            <thead class="text-xs text-slate-400 uppercase bg-slate-50 border-b border-slate-200 font-bold">
              <tr>
                ...
                <th class="px-6 py-4">Termin P³atnoœci</th>
                <th class="px-6 py-4 text-red-600 font-bold">OpóŸnienie</th>
            ...
            <tbody class="divide-y divide-slate-100 text-slate-600">
              <tr v-if="overdueInvoices.length === 0" class="bg-white">
                <td colspan="7" class="px-6 py-6 text-sm text-slate-500">Brak zaleg³oœci p³atniczych.</td>
              </tr>
              <tr v-for="inv in paginatedOverdueInvoices" :key="inv.id" class="hover:bg-red-50/10 transition bg-white group">
                ...
                <td class="px-6 py-4 text-right whitespace-nowrap"><button type="button" class="text-xs font-bold text-white bg-red-500 hover:bg-red-600 px-4 py-2 rounded-lg transition shadow-sm">Szczegó³y</button></td>
              </tr>
            </tbody>
          </table>
        </div>
    ...
    <Teleport to="body">
      <div v-if="isNewsModalOpen && selectedNews" class="fixed inset-0 z-9999 flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-sm" @click.self="closeNewsModal">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto transform transition-all p-8 relative">
          ...
          <div class="prose prose-slate max-w-none text-slate-700 leading-relaxed max-h-[60vh] overflow-y-auto mb-6" v-html="selectedNews.content || '<i>Brak treœci</i>'"></div>
          ...
          <div class="mt-8 border-t border-slate-100 pt-6 flex justify-between items-end">
            <div v-if="selectedNews.attachment" class="flex flex-col gap-2">
              ...
              <a :href="selectedNews.attachment" target="_blank" class="inline-flex items-center gap-2 text-stratton-gold font-bold hover:text-amber-700 transition group p-2 rounded-lg bg-amber-50 hover:bg-amber-100 border border-amber-100">
                ...
                <span class="text-xs uppercase tracking-wide">{{ selectedNews.attachmentName || 'Pobierz za³¹cznik' }}</span>
              </a>
            </div>
