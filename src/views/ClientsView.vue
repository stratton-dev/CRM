...
const selectClient = (client: Client) => {
  ...
  if (auth.enabled) {
    ...
    // LEADOWIEC widzi kalkulacje read-only (backend scope po added_by_user_id),
    // ale NIE ma dostępu do zgód (zarządza nimi agent) — pomijamy je, by nie 403.
          ...
          <div class="flex-1 p-2 overflow-y-auto space-y-2" @dragover="onDragOver" @drop="onDrop($event, stage.status)">
            ...
            >
              ...
              <div class="text-xs text-slate-400 mt-3 flex justify-between items-center">
                ...
                <button type="button" class="px-2.5 py-1 text-[11px] font-bold rounded-md bg-emerald-50 text-emerald-700 border border-emerald-200 hover:bg-emerald-100 transition" title="Pokaż szczegóły" @click.stop="selectClient(client)">
                  Szczegóły
                </button>
              </div>
            </div>
