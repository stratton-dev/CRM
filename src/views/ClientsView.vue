...
const downloadCalculationExcel = async (calc: any) => {
  if (!calc.valueJson) {
    toast.warning('Ta kalkulacja nie posiada zapisanych danych szczegółowych.')
    return
  }
  try {
    const data = calc.valueJson
    // data should contain firma, pracownicy, config, wyniki
    await excelGenerator.generateDetailedReport({
      firma: data.firma,
      wyniki: {
        szczegoly: [], // The Excel generator expects szczegoly for the detailed tab
        podsumowanie: data.wyniki,
      },
      prowizjaProc: data.prowizjaProc || 26,
    })
    toast.success('Pobieranie raportu Excel...')
  } catch (error) {
