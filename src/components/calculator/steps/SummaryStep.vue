...
const tableModelLabel = computed(() =>
  isPlus.value ? 'Eliton Prime™ PLUS' : `Eliton Prime™ ${store.prowizjaProc}%`
);
...
const tPodwyzka       = computed(() => isPlus.value ? tableScope.value.reduce((a, w) => a + w.podzial.swiadczenie.netto * 0.04, 0) : 0);
const tAdminBonus     = computed(() => tableScope.value.reduce((a, w) => a + w.podzial.swiadczenie.netto * 0.02, 0));
          ...
          <table class="w-full text-sm">
            ...
            <tbody>
              ...
              <tr v-if="isPlus" class="border-b border-slate-50 hover:bg-blue-50/30 transition-colors group">
                <td class="px-5 py-3 text-[12px] font-medium text-slate-700">
                  <div class="flex items-center gap-2">
                    <div class="w-1 h-4 rounded-full bg-sky-400 shrink-0"></div>
                    Podwyżka dla pracowników <span class="text-slate-400 font-normal">(4% świadczeń)</span>
                    <span class="text-[9px] font-black uppercase tracking-widest text-sky-600 bg-sky-50 border border-sky-200 px-1.5 py-0.5 rounded">PLUS</span>
                  </div>
                </td>
                <td class="px-5 py-3 text-right text-slate-300 tabular-nums text-[12px]">—</td>
                <td class="px-5 py-3 text-right font-bold text-sky-600 tabular-nums text-[12px] bg-amber-50/40">{{ formatPLN(tPodwyzka) }}</td>
                <td class="px-5 py-3 text-right font-black text-sky-600 tabular-nums text-[12px] bg-emerald-50/40">+{{ formatPLN(tPodwyzka) }}</td>
              </tr>
              <tr class="border-b border-slate-50 hover:bg-blue-50/30 transition-colors group">
                <td class="px-5 py-3 text-[12px] font-medium text-slate-700">
                  <div class="flex items-center gap-2">
